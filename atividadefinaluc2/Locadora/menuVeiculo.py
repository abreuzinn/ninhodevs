...
def showVeiculoEspecifico(conex):
    sleep(1)
    ref = input("\nInsira o código de referência (REF) de algum veículo do catálogo: ")
...
def showVeiculosMarca(conex):
    ...
    while True:
        sleep(1)
        marcaVEICULO = input("\nInsira o nome da marca desejada (utilize 0 para voltar): ")
        match marcaVEICULO:
            case "0":
                break
            case other:
...
def showVeiculosLitragem(conex):
    ...
    while True:
        sleep(1)
        litragemVEICULO = input("\nInsira a litragem desejada (utilize 0 para voltar): ")
...
def showVeiculosAno(conex):
    ...
    sleep(1)
    print("\nANOS (fabricação/modelo)")
    for ano in resultado:
        print(f"- {ano[0]}/{ano[1]}")
        
    while True:
        sleep(1)
        escolhaAno = input("\nVocê deseja consultar os veículos por ano de fabricação ou do modelo?\n\n1- Fabricação\n2- Modelo\n0- Voltar\n\n")
        match escolhaAno:
            case "1":
                sleep(1)
                anoVEICULO = input("\nInsira o ano desejado (insira 0 para voltar): ")
                match anoVEICULO:
                    case "0":
                        break
                    case other:
...
def showVeiculosCarroceria(conex):
    ...
    while True:
        sleep(1)
        carroceriaVEICULO = input("\nInsira o tipo de carroceria desejada (insira 0 para voltar): ")
...
def addVeiculo(conex):
    ...
    while True:
        ...
        if anoFabVeiculo.isnumeric():
            if len(anoFabVeiculo) == 4:
                break
            else:
                sleep(1)
                print("\nO ano deve possuir 4 algarismos.")
        else:
            sleep(1)
            print("\nInsira o ano de forma numérica (exemplo: 2018)")
    while True:
        anoModVeiculo = input("Insira o ano do modelo do veículo: ")
        if anoModVeiculo.isnumeric():
            if len(anoModVeiculo) == 4:
                break
            else:
                sleep(1)
                print("\nO ano deve possuir 4 algarismos.")
